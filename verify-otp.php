...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!Security::verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if (empty($otp_code)) {
            ...
            if ($otp_record) {
                // Mark OTP as used
                $db->update('otp_codes', ['is_used' => 1], 'id = ?', [$otp_record['id']]);
...
if (isset($_GET['resend']) && $_GET['resend'] === '1') {
    if (!Security::checkRateLimit('resend_otp_' . getUserIP(), 3, 300)) {
        ...
        $db->query(
            "UPDATE otp_codes SET is_used = 1 WHERE email = ? AND purpose = 'registration'",
            [$email]
        );
        ...
        // Send OTP using Auth SMTP
        $emailHelper = new EmailHelper('auth');
        if ($emailHelper->sendOTP($email, $otp, $pending_registration['full_name'], 'user')) {
            Session::setFlash('success', 'New OTP sent to your email.', 'success');
        } else {
            Session::setFlash('warning', 'OTP generated but email may not have been sent.', 'warning');
        }
    }
        ...
        <div class="col-md-5">
            <div class="card shadow">
                <div class="card-body p-5">
                    ...
                    if ($flash_warning):
                    ?>
                    <div class="alert alert-<?= $flash_warning['type'] ?> alert-dismissible fade show">
                    ...
                    <div class="alert alert-info text-center py-2">
                        <small><strong>Dev Mode:</strong> Check database for OTP or use 
                        <a href="<?= BASE_URL ?>/api/auth/get-otp-debug.php?email=<?= urlencode($email) ?>" target="_blank">get OTP</a>
                        </small>
                    </div>
                    <?php endif; ?>
